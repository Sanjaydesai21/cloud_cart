...
aws s3 mb s3://cloudcart-tfstate-194226864100 --region us-east-1
aws s3api put-bucket-versioning \
  --bucket cloudcart-tfstate-194226864100 \
  --versioning-configuration Status=Enabled

# Put your real bucket name into backend.tf (replace 194226864100)
nano backend.tf
...
aws ecr get-login-password --region us-east-1 | \
  docker login --username AWS --password-stdin 194226864100.dkr.ecr.us-east-1.amazonaws.com
...
pointing `cloudcart.sanjaydesai.xyz` at CloudFront.

```bash
curl -I https://cloudcart.sanjaydesai.xyz/assets/main.abc123.js   # X-Cache: Hit from cloudfront (2nd request)
curl -I https://cloudcart.sanjaydesai.xyz/api/products            # X-Cache: Miss from cloudfront (always)
```
...
aws s3 rb s3://cloudcart-tfstate-194226864100 --force
```
